...
function getContrastColor(hex: string, theme: string): string {
...
// 模块加载时生成一次随机几何参数（不随渲染变化），满足 React 纯函数规则
...
const PARTICLE_GEOMETRY: ParticleGeometry[] = Array.from({ length: 48 }).map(
  (_, i) => {
...
const CompletionCelebration = ({ show, scheme }: CompletionCelebrationProps) => {
  ...
  const palette = useMemo(() => {
    ...
    const key: "dark" | "light" = resolvedTheme === "dark" ? "dark" : "light";
    const base = schemeColors?.[key] ?? DEFAULT_COLORS;
    return base.map((c) => getContrastColor(c, resolvedTheme ?? "light"));
  }, [scheme, resolvedTheme]);

  const particles = useMemo<Particle[]>(
    () =>
      PARTICLE_GEOMETRY.map((g) => ({
        ...g,
        color: palette[g.id % palette.length],
      })),
    [palette]
  );
