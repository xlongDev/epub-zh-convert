...
import { buildConvertedZip } from "@/utils/zipUtils";
...
// 文件大小格式化函数（统一引用 utils/format，避免与 FileList 重复且不一致）
...
const ConvertedFilesList = ({
  ...
  // 修改单文件下载，添加音效
  const wrappedHandleDownloadSingle = (index: number) => {
    playDownloadSound(); // 播放音效
    handleDownloadSingle(index);
  };

  // 批量下载所选文件（含音效），打包逻辑复用共享的 buildConvertedZip
    ...
    try {
      await buildConvertedZip(
        selectedIndices.map((i) => ({
          ...
          blob: convertedFiles[i].blob,
        })),
        `批量下载文件(${selectedIndices.length}个).zip`
      );
      setSelectedFiles(new Set());
    } catch (error) {
  ...
  // 下载所有文件（含音效）：
  // 单文件走单文件下载；多文件走 store 的 handleDownloadAll（已统一复用 buildConvertedZip）
