...
/**
 * 📦 DirectionSelector - 转换方向选择「容器组件」
 * 直接从全局 store 读取 direction / setDirection。
 */
const DirectionSelector = React.memo(() => {
    ...
    >
      <div className="relative bg-white/20 dark:bg-gray-800/30 backdrop-blur-lg rounded-xl p-1 border border-white/20 dark:border-gray-700/50 shadow-[0_8px_32px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.2)]">
        ...
        <div className="relative flex z-10">
          {[
            { value: "t2s" as ConversionDirection, label: "繁体转简体" },
            { value: "s2t" as ConversionDirection, label: "简体转繁体" },
          ].map((option) => (
            <motion.button
              key={option.value}
